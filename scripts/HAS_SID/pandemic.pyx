...
cdef class Pandemic:
	def __init__(self, Network network, double beta, list init_S, list init_I, double warm_up):
	...
	cdef public:
		...
		cpdef void infection(self, double u, double t):
			...
			lambda_exp = agent_I.parameters["lambda_plus"]*agent_S.parameters["lambda_plus"] + agent_I.parameters["lambda_minus"]*agent_S.parameters["lambda_minus"]
			c1 = agent_I.parameters["lambda_plus"]*agent_S.parameters["lambda_plus"]/(lambda_exp)

			# get time between last observation
			delta_t = t - self.network.check_edge(agent_I.id, agent_S.id)
		...
		cpdef void recovery(self, double u):
			...
			# update diagnosis and recovery risk
