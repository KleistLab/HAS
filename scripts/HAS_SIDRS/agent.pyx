cdef class Agent:
	def __init__(self, int id, str status, double lambda_plus, double lambda_minus, double inf_i, double diag_i, double rec_i, double sus_i): 	#, double sus_i):
		self.id = id
		self.status
		self.parameters = {'lambda_plus': lambda_plus, 'lambda_minus': lambda_minus,
						 'inf_i': inf_i,'diag_i': diag_i, 'rec_i':rec_i, 'sus_i':sus_i} # 	, 'sus_i': sus_i}
	...
	cdef public:
		...
		cpdef void become_susceptible(self):
			self.status = "S"
			self.inf_rate = self.parameters["inf_i"]
